...
def main(args):
    ...
    test = load_csv(test_path, get_html, total=16039)
    vectorizer.fit([item.features for item in test])
    train = load_csv(train_path, get_html, total=7044)
    ...
    # test = load_csv(test_path, get_url, total=16039)
    # vectorizer_url.fit([item.features for item in test])
    # train = load_csv(train_path, get_url, total=7044)
    ...
    train = load_csv(train_path, skip_feature, total=7044)
    ...
    with open('submission.txt', 'w') as f:
        ...
        test = load_csv(test_path, skip_feature, total=16039)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', type=str, help="path to train", required=True)
    parser.add_argument('--test', type=str, help='path to test', required=True)
